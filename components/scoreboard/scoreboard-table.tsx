          ...
          <Table>
            <TableHeader className="text-muted-foreground bg-gray-800/30 backdrop-blur-sm">
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow
                  key={headerGroup.id}
                  className="grid grid-cols-[80px_1fr_2fr_1fr] border-b border-gray-700/50">
              ...
              {table.getRowModel().rows?.length ? (
                table.getRowModel().rows.map((row, index) => (
                  <motion.tr
                    ...
                    key={row.id}
                    className="grid grid-cols-[80px_1fr_2fr_1fr] text-color-light hover:bg-gray-800/30 transition-colors">
